...
if [[ ! -f "$OCTALUM-PULSE" ]]; then
  echo "pulse missing" >&2; exit 1;
fi

# Test 1: default invocation (may skip if tools absent)
test_run "default-run" bash "$OCTALUM-PULSE" scanners

# Test 2: disable lynis
test_run "no-lynis" env SCAN_LYNIS=false bash "$OCTALUM-PULSE" scanners

# Test 3: disable rkhunter
test_run "no-rkhunter" env SCAN_RKHUNTER=false bash "$OCTALUM-PULSE" scanners

# Test 4: both disabled (should still produce summary file)
test_run "none" env SCAN_LYNIS=false SCAN_RKHUNTER=false bash "$OCTALUM-PULSE" scanners
...
# Note: Threshold checking may not be implemented in pulse yet
# This test verifies behavior when LYNIS_MIN_SCORE is set
if command -v lynis >/dev/null 2>&1; then
  # For now, we just verify pulse runs without crashing
  # TODO: Implement actual threshold checking in pulse
  if env LYNIS_MIN_SCORE=999 SCAN_RKHUNTER=false bash "$OCTALUM-PULSE" scanners >/dev/null 2>&1; then
...
if env LYNIS_MIN_SCORE=0 bash "$OCTALUM-PULSE" scanners >/dev/null 2>&1; then
  pass "threshold-pass"
else
  fail "threshold-pass"
fi
...
if [[ -f "$TEMP_OLD" ]]; then
  ...
  env RETENTION_DAYS=30 bash "$OCTALUM-PULSE" scanners >/dev/null 2>&1 || true
  if [[ ! -f "$TEMP_OLD" ]]; then
...
if env PATH=/nonexistent SCAN_LYNIS=true SCAN_RKHUNTER=true bash "$OCTALUM-PULSE" scanners >/dev/null 2>&1; then
