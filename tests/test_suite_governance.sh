...
if echo "$output" | grep -qE "(Security audit|checking|sudoers)"; then
...
output=$(DRY_RUN=true JSON_SUMMARY=false bash "$SYSMAINT" --dry-run --security-audit 2>&1 || true)
if echo "$output" | grep -qE "(shadow|sudoers|checking)"; then
...
if echo "$output" | grep -q "System maintenance started" && echo "$output" | grep -q "System maintenance finished"; then
...
if echo "$output" | grep -qE "(root|permission|privilege|DRY RUN)"; then
...
output=$(DRY_RUN=true JSON_SUMMARY=false bash "$SYSMAINT" --dry-run --upgrade 2>&1 || true)
if echo "$output" | grep -qE "(DRY RUN|would|simulation)"; then
